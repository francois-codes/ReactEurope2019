...
class HomeLatestStraight extends PureComponent {
  ...
  renderGroup(group) {
    const { match, values, groupTitle } = group;
    return (
  ...
  isNearlyToday(item) {
    ...
    return numberOfDays >= -1 && numberOfDays <= 2;
  }
...
export default connect(mapStateToProps, mapDispatchToProps)(HomeLatestStraight);
